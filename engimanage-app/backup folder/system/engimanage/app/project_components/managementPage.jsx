...
const ManagementPage = ({ projectID }) => {
  ...
  const toggleSelectMember = (member) => {
    ...
    if (isSelected) {
      setSelectedMembers((prev) => prev.filter((m) => m.fname !== member.fname));
    } else {
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.card}>
        ...
        <Text style={styles.sectionContent}>{teamLeader.replace(/_/g,' ')}</Text>
      </View>
      ...
      <View style={styles.card}>
        ...
        {members?.map((member) => (
          <View key={member.ID} style={styles.memberRowAligned}>
            <Image 
              source={{ uri: member.profile_image ? `${link.serverLink}/${member.profile_image}` : `${link.serverLink}/profile/user.png` }} 
              style={styles.memberImage}
            />
            <Text style={styles.memberText}>
              {member.fname.replace(/_/g,' ')} {member.lname.replace(/_/g,' ')} - {member.role.replace(/_/g,' ')}
            </Text>
            {isManager && (
              <TouchableOpacity onPress={() => alert(member.ID)} style={styles.trashButton}>
