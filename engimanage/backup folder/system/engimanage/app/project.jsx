...
const Project = () => {
  ...
  const handleProjectClick = async (projectID) => {
    ...
    try {
      ...
      if(data.length >= 1){
        router.navigate(`/project_components/projectHandler?projectID=${projectID}`);
      }else{
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={projects.length > 0 ? {display:"none"} : {color:"red",marginTop:200}}>{"~No current projects"}</Text>
      ...
      <ScrollView style={{ width: "100%" }}>
        {getFilteredProjects().map((project) => (
          <ProjectPanel
            ...
            txtstyle={{ fontSize: 20, fontWeight: "bold", color: "black", marginBottom: 10 }}
      ...
      <TouchableOpacity
        ...
        <View>
          <Text style={{color:'#fff',fontSize:15,fontWeight:'bold'}} >{userRole === "project_manager" ? 'Add Project' : 'Join Project'}</Text>
        </View>
      </TouchableOpacity>
      ...
      <Dialog.Container visible={joinpopUp}>
        ...
        <Dialog.Input label="Pin" onChangeText={setJoinpin}></Dialog.Input>
        ...
        />
        {/*how to print the project id and user id in here */}
        <Dialog.Button label="Request Join" onPress={()=> send_request_join(selectedProjectID,userID)}/>  
